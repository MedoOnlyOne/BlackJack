...
fn =function(){
    if (product_name.value.length>5 && stock.value!='' && stock.value!='' && description.value.length>15 && image.value!='' && ( image.value.slice(image.value.length-3)=='jpg' || image.value.slice(image.value.length-3)=='png'))
        {
